...
while command != "End":
    ...
    action = command[0]
    index = int(command[1])
    attribute = int(command[2])

    if action == "Shoot":
        if 0 <= index < len(targets):
            targets[index] -= attribute
            if targets[index] <= 0:
                del targets[index]

    elif action == "Add":
        if 0 <= index < len(targets):
            targets.insert(index, attribute)
        else:
            print("Invalid placement!")

    elif action == "Strike":
        if index - attribute >= 0 and index + attribute < len(targets):
            del targets[index - attribute:index + attribute + 1]
        else:
            print("Strike missed!")
